            ...
            for line in f:
                ...
                if len(line) > 0 and not (line[0] == "#"):
                    if "\t" in line and (line.find("\t")+1 < len(line)):
                        ...
                        frequency = int(line[0:line.find("\t")])
                        rank += 1
                        new_entry = WordListEntry(word, frequency, rank)
    ...
    def rank(self, words, limit=0):
        ...
        for word in words:
            if not (self.get_rank(word) == -1):
                new_entry = WordListEntry(word, self.get_frequency(word), self.get_rank(word))
                ranked_list.append(new_entry)
            else:
                new_entry = WordListEntry(word, 9999999999, 9999999999)
                ranked_list.append(new_entry)
