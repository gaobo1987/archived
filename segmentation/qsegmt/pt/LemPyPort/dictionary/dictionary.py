from qsegmt.pt.LemPyPort.dictionary.dictionary_entry import *
...
class Dictionary:
        ...
        with open(input_file, "r") as f:
            for line in f:
                if len(line) > 0 and line[0] != "#":
                    if "." in line and "," in line and line.find(".") > line.find(",") + 1 \
                            and line.find(".") + 1 < len(line) and \
                            ((line[line.find(".")+1] != ":") or (line[line.find(".")+1] != "+")):
                        ...
                        entry = DictionaryEntry(inflected_form, lemma, part_of_speech, subcategory, morph_attributes)
                        self.add(entry)
    ...
    def remove(self, inflected_form, part_of_speech=None):
    ...
    def contains(self, inflected_form, part_of_speech=None):
        if not part_of_speech:
    ...
    def retrieve_lemas(self, inflected_form, part_of_speech):
        lemas = []
        if inflected_form in self.dictionary_list:
            entry = self.dictionary_list[inflected_form]
            for elem in entry:
    ...
    def retrieve_entries(self, inflected_form):
        entries = []
        if inflected_form in self.dictionary_list:
            entry = self.dictionary_list[inflected_form]
            for elem in entry:
...
if __name__ == "__main__":
    ...
    print(novo_dict.retrieve_lemas("abelha", "N"))
    print(novo_dict.contains("abelha", "N"))
    print(novo_dict.retrieve_lemas("abelha", "V"))
    print(novo_dict.retrieve_entries("abelha"))
