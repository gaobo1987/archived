...
import datasets
from datasets import BuilderConfig, GeneratorBasedBuilder, ArrowReader, Split, Dataset, DatasetInfo
...
class WikiDprConfig(BuilderConfig):
...
class WikiDpr(GeneratorBasedBuilder):
    ...
    def _post_process(self, dataset, resources_paths):
        print('---wiki_dpr.py:185, _post_process: begin, self.config.with_index=', self.config.with_index)
        if self.config.with_index:
            index_file = resources_paths["embeddings_index"]
            if os.path.exists(index_file):
                print('---wiki_dpr.py:188, _post_process: index_file already exists:')
                print(index_file)
                dataset.load_faiss_index("embeddings", index_file)
            else:
                print('---wiki_dpr.py:188, _post_process: index_file not exists')
